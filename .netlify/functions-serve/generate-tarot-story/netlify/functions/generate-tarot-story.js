...
// netlify/functions/generate-tarot-story.js
var generate_tarot_story_exports = {};
__export(generate_tarot_story_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(generate_tarot_story_exports);
...
var openai = new openai_default({
  ...
  // Ensure OPENAI_KEY is set in .env
});
...
`;
    cards.forEach((card, index2) => {
      const phase = phases[index2] || `Phase ${index2 + 1}`;
      storyPrompt += `
            Phase: ${phase}
            Card: ${card.name}
...
*/
//# sourceMappingURL=generate-tarot-story.js.map
